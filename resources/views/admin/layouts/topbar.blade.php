    ...
    <li class="dropdown pc-h-item">
      <a class="pc-head-link dropdown-toggle me-0" data-pc-toggle="dropdown" href="#" role="button"
         aria-haspopup="false" aria-expanded="false">
          ...
          @php($unreadMsg = \App\Models\AdminMessage::where('is_read', false)->count())
          @if($unreadMsg > 0)
              <span class="badge bg-warning-500 text-white rounded-full z-10 absolute right-0 top-0">{{ $unreadMsg }}</span>
          @endif
      </a>
      <div class="dropdown-menu dropdown-notification dropdown-menu-end pc-h-dropdown p-2">
          <div class="dropdown-header flex items-center justify-between py-4 px-5">
              <h5 class="m-0">Admin Messages</h5>
              <a href="{{ route('admin.messages.msg') }}" class="btn btn-link btn-sm">View all</a>
          </div>
          <div class="dropdown-body header-notification-scroll relative py-4 px-5" style="max-height: calc(100vh - 215px)">
              @foreach(\App\Models\AdminMessage::latest()->take(5)->get() as $msg)
                  <a href="{{ route('admin.message.read', $msg->id) }}" class="dropdown-item">
                      <strong>{{ $msg->title }}</strong>
                      <p>{{ Str::limit($msg->message, 80) }}</p>
