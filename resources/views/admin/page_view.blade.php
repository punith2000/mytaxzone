  ...
  <body>
    ...
    <div class="pc-container">
      <div class="container mt-4">
        <h3 class="mb-3">Page Views Logs
          <a href="{{ route('admin.dashboard') }}"> <i class="bi bi-arrow-left-circle float-end"></i> </a>
        </h3>
        <p>Total Views: <strong>{{ $totalViews }}</strong></p>
          ...
          <tbody>
            @foreach($pageviews as $views)
              <tr>
                <td>{{ $views->id }}</td>
                <td>{{ $views->page }}</td>
                <td>{{ Str::limit($views->view, 60) }}</td>
                <td>{{ $views->visited_at }}</td>
              </tr>
            @endforeach
          </tbody>
        </table>
